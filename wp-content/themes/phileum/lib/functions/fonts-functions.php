...
"Helvetica","Lucida Grande","Courier New","Lucida Console","Impact","Georgia","Palatino Linotype","Museo","Novecento",
"---- Google Web Font ----",
...
if( !function_exists('wp_get_google_webfonts_family_list') ):
function wp_get_google_webfonts_family_list($key='', $sort='') {
$http = (!empty($_SERVER['HTTPS'])) ? "https" : "http";
if(!$key):
$key = 'AIzaSyBmPRa5TGlBRbUUV-pVPU3GxXRkD4lBtUU';
else:
$key = $key;
endif;
...
$fontlist = wp_get_google_webfonts_family_list('AIzaSyBmPRa5TGlBRbUUV-pVPU3GxXRkD4lBtUU', 'alpha');
...
if ($bodytype == "" || $bodytype == "Choose a font" || in_array($un_bodytype, $safe_font) ){  ?>
<?php } else {
echo "<link href='" . $http . "://fonts.googleapis.com/css?family=" . $bodytype . ":" . $font_var . "' rel='stylesheet' type='text/css'>";
}

if ($headtype == "" || $headtype == "Choose a font" || in_array($un_headtype, $safe_font)){ ?>
<?php } else {
echo "<link href='" . $http . "://fonts.googleapis.com/css?family=" . $headtype . ":" . $font_var . "' rel='stylesheet' type='text/css'>";
}

if ($navtype == "" || $navtype == "Choose a font" || in_array($un_navtype, $safe_font)){  ?>
<?php } else {
echo "<link href='" . $http . "://fonts.googleapis.com/css?family=" . $navtype . ":" . $font_var . "' rel='stylesheet' type='text/css'>";
} ?>
...
<?php if($bodytype=='Museo' || $headtype=='Museo' || $navtype=='Museo'): ?>
<?php get_template_part ( '/lib/fonts/museo300/museo300' ); ?>
<?php endif; ?>
<?php if($bodytype=='Novecento' || $headtype=='Novecento' || $navtype=='Novecento'): ?>
<?php get_template_part ( '/lib/fonts/novecento/novecento' ); ?>
<?php endif; ?>
<?php print "</style>"; ?>
<?php }

add_action('wp_head', 'get_fontface_list');
