...
add_action("admin_print_scripts", 'inkthemes_load_only');
/* ----------------------------------------------------------------------------------- */
...
function inkthemes_load_only() {
    ...
    function of_admin_head() {
        ...
        echo '<link rel="stylesheet" type="text/css" href="' . get_template_directory_uri() . '/admin/css/admin-style.css" media="screen" />';
        ...
        foreach ($options as $option) {
            if ($option['type'] == 'color' OR $option['type'] == 'typography' OR $option['type'] == 'border') {
                if ($option['type'] == 'typography' OR $option['type'] == 'border') {
                    $option_id = $option['id'];
                    $temp_color = cc_get_option($option_id);
                    $option_id = $option['id'] . '_color';
                    $color = $temp_color['color'];
                } else {
                    $option_id = $option['id'];
                    $color = cc_get_option($option_id);
                }
                ?>
        ...
        //AJAX Upload
        ?>
        <script type="text/javascript" src="<?php echo get_stylesheet_directory_uri(); ?>/admin/js/ajaxupload.js"></script>
...
function inkthemes_ajax_callback() {
    ...
    if ($save_type == 'upload') {
        ...
        cc_update_option($clickedID, $uploaded_file['url']);
                ...
                if (is_array($type)) {
                    foreach ($type as $array) {
                        if ($array['type'] == 'text') {
                            ...
                            cc_update_option($id, stripslashes($new_value));
                        }
                    }
                } elseif ($new_value == '' && $type == 'checkbox') { // Checkbox Save
                    cc_update_option($id, 'false');
                } elseif ($new_value == 'true' && $type == 'checkbox') { // Checkbox Save
                    cc_update_option($id, 'true');
                    ...
                    foreach ($option_options as $options_id => $options_value) {
                        ...
                        if (!isset($output[$multicheck_id])) {
                            cc_update_option($multicheck_id, 'false');
                        } else {
                            cc_update_option($multicheck_id, 'true');
                        }
                    }
                    ...
                    cc_update_option($id, $typography_array);
                } elseif ($type == 'border') {
                    ...
                    cc_update_option($id, $border_array);
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case 'select':
                ...
                $select_value = cc_get_option($value['id']);
            ...
            case "typography":
                ...
                $default = $value['std'];
                $typography_stored = cc_get_option($value['id']);
            ...
            case "border":
                ...
                $default = $value['std'];
                $border_stored = cc_get_option($value['id']);
