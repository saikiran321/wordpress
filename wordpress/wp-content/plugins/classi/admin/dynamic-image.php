 ...
 * @return mixed|string
 */
function cc_document_root($path) {
 ...
 */
function cc_image_resize($img_url, $width, $height, $crop = true, $quality = 100) {
    ...
    if ($_SERVER['HTTP_HOST'] != $file_path['host'] && $file_path['host'] != '') {  // The image is not locally hosted
        ...
        $file_path = $copy_to_file;
			...
			if( $findit===false){
			$file_path = cc_document_root($file_path['path']) . $file_path['path'];
			}
			else{
				$expath=$file_path['path'];		
 ...
 * @return type 
 */
function cc_thumbnail_resize($attach_id = null, $img_url = null, $width, $height, $crop = false, $jpeg_quality = 90) {
